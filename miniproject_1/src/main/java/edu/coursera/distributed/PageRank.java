...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer, Double> newRanks =
                sites
                        ...
                        .flatMapToPair(
                                kv -> {
                                    Integer id = kv._1();
                                    Tuple2<Website, Double> val = kv._2();
                                    Website edge = kv._2()._1();
                                    Double curRank = kv._2()._2();


                                    LinkedList<Tuple2<Integer, Double>> contribs = new LinkedList<>();

                                    Iterator<Integer> iter = edge.edgeIterator();

                                    while (iter.hasNext()) {
                                        final int target = iter.next();

                                        contribs.add(new Tuple2(target, curRank / (double) edge.getNEdges()));
                                    }
        ...
        return newRanks
                .reduceByKey((aDouble, aDouble2) -> aDouble + aDouble2)
                .mapValues(aDouble -> 0.15 + 0.85 * aDouble);
    }
}
